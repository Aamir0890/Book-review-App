...
router.route("/asc").post((req,res)=>{
         db.query(`SELECT * FROM booksreviewdb.books ORDER BY BookName ASC`,
         (error,result)=>{
...
router.route("/desc").post((req,res)=>{
    db.query(`SELECT * FROM booksreviewdb.books ORDER BY BookName DESC`,
    (error,result)=>{
...
router.route("/description/:id").get((req,res)=>{
    let averageRating=0;
    let reviews=[];
    db.query(`SELECT * FROM booksreviewdb.books where id=${parseInt(req.params.id)}`,
    (error,result)=>{
        db.query(`SELECT * FROM booksreviewdb.rating where id=${parseInt(req.params.id)}`,
        (error,result_rating)=>{
             if(result_rating.length >0){
                averageRating=result_rating.reduce((acc,currval)=>{
                    return acc + currval.rating;
                },0)/result_rating.length;
             }
        } 
        ) 
        db.query(`SELECT * FROM booksreviewdb.review where id=${parseInt(req.params.id)}`,
          (error,result_review)=>{
 ...
 router.route("/delete").post((req,res)=>{
    db.query(`DELETE FROM booksreviewdb.books where id=${parseInt(req.body.id)}`,(error,result)=>{
        db.query(`SELECT * FROM booksreviewdb.books`,(error,result)=>{
            return res.render("index",{ReviewList:result})
        })
    })
 ...
 router.route("/addRating").post((req,res)=>{
    ...
    db.query(`INSERT INTO booksreviewdb.rating (bookId, rating) VALUES ("${bookId}","${rating}")`,
    (error,result)=>{
        if(error === null){
 ...
 router.route("/addReview").post((req,res)=>{
    ...
    db.query(`INSERT INTO booksreviewdb.review (bookId,review) VALUES ("${bookId}","${review}")`,
    (error,result)=>{
        if(error===null){
